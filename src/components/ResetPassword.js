...
const ResetPassword = (props) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://memomate-backend-jid4.onrender.com/api/auth/reset-password/${token}`, {
        method: "POST",
        headers: {
      ...
      props.showAlert('An error occurred during password reset.', 'danger');
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900">
      <div className="w-full max-w-sm mx-auto px-4 py-8 bg-gray-800 rounded-md shadow-md">
        <h2 className="text-center text-white text-2xl mb-6" style={paragraphStyle}>
          RESET PASSWORD
        </h2>
